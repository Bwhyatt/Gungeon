...
class TheWall():
    ...
    def WallCollision(self, obj2):
        ...
        if(self.Rect.colliderect(obj2.Rect)):
            if(self.pos[1] > obj2.pos[1] + obj2.Rect.h):
                obj2.BannedDirections.append(1)#it means that the wall is below the bottom of the player
            if(self.pos[1] + self.Rect.h < obj2.pos[1]):
                obj2.BannedDirections.append(3)
            if(self.pos[0] > obj2.pos[0] + obj2.Rect.w):
                obj2.BannedDirections.append(2)
            if(self.pos[0] + self.Rect.w < obj2.pos[0]):
